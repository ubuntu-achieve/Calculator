...
bool wfile(std::string answer) {
	...
	outfile << (char*)answer.c_str();
	outfile.close();
	return true;
}
extern"C" __declspec(dllexport) void out_add(const char* num1, const char* num2) {
	BigNumber answer = BigNumber(num1) + BigNumber(num2);
	wfile(answer.get());
}

extern"C" __declspec(dllexport) void out_minus(const char* num1, const char* num2) {
	BigNumber answer = BigNumber(num1) - BigNumber(num2);
	wfile(answer.get());
}
extern"C" __declspec(dllexport) void out_times(const char* num1, const  char*num2) {
	BigNumber answer = BigNumber(num1) * BigNumber(num2);
	wfile(answer.get());
}

extern"C" __declspec(dllexport) void out_div(const char* num1, const char* num2) {
	BigNumber answer = BigNumber(num1) / BigNumber(num2);
	wfile(answer.get());
}

extern"C" __declspec(dllexport) void out_pow(const char* num1, int num2) {
	BigNumber answer = BigNumber(num1).pow(num2);
	wfile(answer.get());
}

/*int main() {
	out_add("123", "234");
	out_minus("123.12", "123");
	out_pow("8", 2);
	return 0;
}*/